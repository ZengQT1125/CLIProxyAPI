...
//
// Active cooldowns remain authoritative across model re-registration. Other
// supported-model errors are reset because re-registration already cleared the
// registry-side suspension snapshot. ModelStates for models that are no longer
...
func (m *Manager) setConfigSnapshotLocked(cfg *internalconfig.Config) bool {
	...
	clearedCooldowns := len(clearedCooldownAuthIDs) > 0
...
// PrepareCooldownRestore loads persisted cooldown records before auth registration begins.
...
// CompleteCooldownRestore reconciles persisted cooldown records after auth registration.
...
func (m *Manager) persistCooldownStatesToLocked(ctx context.Context, store CooldownStateStore) bool {
	...
	snapshots := cooldownRecordsToSnapshots(records)
	if errApply := store.Apply(ctx, snapshots); errApply != nil {
		logEntryWithRequestID(ctx).Warnf("failed to persist cooldown state: %v", errApply)
		return false
	}
	return ctx.Err() == nil
}

// cooldownRecordsToSnapshots groups flat records by AuthID into snapshots.
func cooldownRecordsToSnapshots(records []CooldownStateRecord) []CooldownStateSnapshot {
	byAuth := make(map[string][]CooldownStateRecord, len(records))
	order := make([]string, 0)
	for _, r := range records {
		if _, exists := byAuth[r.AuthID]; !exists {
			order = append(order, r.AuthID)
		}
		byAuth[r.AuthID] = append(byAuth[r.AuthID], r)
	}
	snapshots := make([]CooldownStateSnapshot, 0, len(byAuth))
	for _, authID := range order {
		snapshots = append(snapshots, CooldownStateSnapshot{AuthID: authID, Records: byAuth[authID]})
	}
	return snapshots
}
	...
	go func() {
		...
		emit := func(chunk cliproxyexecutor.StreamChunk) bool {
			if chunk.Err != nil && !failed {
				...
				result := Result{AuthID: auth.ID, Provider: provider, Model: resultModel, Success: false, Error: rerr, RetryAfter: retryAfterFromError(chunk.Err)}
