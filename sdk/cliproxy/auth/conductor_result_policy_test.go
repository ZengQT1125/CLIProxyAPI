...
func (s *recordingCooldownStore) Load(context.Context) ([]CooldownStateSnapshot, error) {
	return nil, nil
}

func (s *recordingCooldownStore) Apply(_ context.Context, snapshots []CooldownStateSnapshot) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.records = nil
...
func TestManager_MarkResult_ResultPolicy_DemotesCredentialScopeBeforeMutationAndPersist(t *testing.T) {
	...
	// Cooldown persistence is batched; flush before asserting what was stored.
	if errFlush := m.FlushCooldownStates(context.Background()); errFlush != nil {
