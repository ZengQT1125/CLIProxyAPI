...
func (s *SessionAffinitySelector) Pick(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*Auth) (*Auth, error) {
	...
	explicitID, _ := extractExplicitSessionIDs(opts.Headers, opts.OriginalRequest, opts.Metadata)
	if explicitID == "" {
	...
	return s.pickWithRetry(ctx, provider, model, opts, available, available)
}
...
func (s *SessionAffinitySelector) pickWithRetry(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, ready, selectable []*Auth) (*Auth, error) {
	...
	if fallbackKey != "" {
		if cachedAuthID, ok := s.cache.Get(fallbackKey); ok {
			if auth := authWithID(eligible, cachedAuthID); auth != nil && authWithID(available, cachedAuthID) != nil && (!isSubagent || allowsSubagentAuthInheritance(auth, model)) {
				bind(auth.ID)
