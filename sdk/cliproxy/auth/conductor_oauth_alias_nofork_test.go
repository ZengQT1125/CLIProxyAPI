...
func TestManager_NoForkAlias_RealPersistenceRestore_BypassesCoolingAuth(t *testing.T) {
	...
	flushCooldownStateForTest(t, manager1)
	...
	restoreStore := &recordingCooldownStateStore{load: initialStore.savedSnapshots()}
...
func (s *strictContextCheckingStore) Apply(ctx context.Context, snapshots []CooldownStateSnapshot) error {
	if ctx == nil {
		return errors.New("nil context")
	}
	if errCtx := ctx.Err(); errCtx != nil {
	...
	for _, snapshot := range snapshots {
		s.records = append(s.records, snapshot.Records...)
	}
	return nil
}

func (s *strictContextCheckingStore) Load(ctx context.Context) ([]CooldownStateSnapshot, error) {
	if ctx == nil {
	...
	byAuth := make(map[string][]CooldownStateRecord)
	for _, record := range s.records {
		byAuth[record.AuthID] = append(byAuth[record.AuthID], record)
	}
	out := make([]CooldownStateSnapshot, 0, len(byAuth))
	for authID, records := range byAuth {
		out = append(out, CooldownStateSnapshot{AuthID: authID, Records: records})
	}
	return out, nil
}
...
func TestManager_NoForkAlias_ResetQuota_ErrorPersistCleansCooldownStore(t *testing.T) {
	...
	snapshotsBefore, errLoadBefore := cooldownStore.Load(context.Background())
	if errLoadBefore != nil || len(snapshotsBefore) == 0 || len(snapshotsBefore[0].Records) == 0 {
		t.Fatalf("expected cooldown records in store before ResetQuota, got: %#v, err: %v", snapshotsBefore, errLoadBefore)
	}
	...
	snapshotsAfter, errLoadAfter := cooldownStore.Load(context.Background())
	if errLoadAfter != nil {
		t.Fatalf("failed to load cooldown store: %v", errLoadAfter)
	}
	if len(snapshotsAfter) != 0 && len(snapshotsAfter[0].Records) != 0 {
		t.Fatalf("expected cooldown store to be cleared after ResetQuota, got: %#v", snapshotsAfter)
	}
