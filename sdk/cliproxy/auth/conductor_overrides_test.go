...
func TestManager_WrapStreamResult_PreservesRetryAfterOnChunkError(t *testing.T) {
	...
	before := time.Now()
	result := m.wrapStreamResult(context.Background(), auth, "codex", model, model, nil, nil, remaining, OAuthModelAliasResult{}, false, cliproxyexecutor.Options{})
...
func TestManager_MarkResult_RequestFaultBodyDoesNotCooldownModelOrAuth(t *testing.T) {
	...
	quotaCooldownDisabled.Store(false)
	t.Cleanup(func() { quotaCooldownDisabled.Store(previous) })
	previousDeleteUnauthorized := deleteUnauthorizedAuth.Load()
	SetDeleteUnauthorizedAuth(true)
	t.Cleanup(func() { SetDeleteUnauthorizedAuth(previousDeleteUnauthorized) })
	...
	if deleted := store.deleted(); len(deleted) != 0 {
		t.Fatalf("expected request-scoped 401 to keep persisted auth, deleted=%v", deleted)
	}
