...
func TestManagerPluginSchedulerAcrossPriorities(t *testing.T) {
	for _, mode := range []string{"round-robin", "session-affinity"} {
		t.Run(mode, func(t *testing.T) {
			var selector Selector = &RoundRobinSelector{}
			if mode == "session-affinity" {
...
// TestManagerSelectAuthByKindAdvancesHomeAuthCount was removed because upstream
// refactored SelectAuthByKind to reject when Home is enabled; the equivalent
// functionality now lives in SelectHomeAuthByKind, and upstream's
// TestManagerLegacySelectAuthFailsClosedWhenHomeEnabled covers the rejection path.
