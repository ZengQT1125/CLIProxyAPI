...
func TestUploadAuthFile_Sub2APIAgentIdentity(t *testing.T) {
	t.Setenv("MANAGEMENT_PASSWORD", "")
	gin.SetMode(gin.TestMode)
	...
	h := NewHandlerWithoutConfigFilePath(&config.Config{AuthDir: authDir}, manager)
	payload := []byte(`{
		"exported_at":"2026-07-22T08:17:13Z",
		"accounts":[{
	...
	rec := uploadRawAuthPayload(t, h, "agent-identity-export.json", payload)
	if rec.Code != http.StatusOK {
		t.Fatalf("upload status = %d, want %d; body = %s", rec.Code, http.StatusOK, rec.Body.String())
	}
	if strings.Contains(rec.Body.String(), "private-key") {
		t.Fatal("upload response exposed agent private key")
	}

	path := filepath.Join(authDir, "codex-agent@example.com.json")
	data, errRead := os.ReadFile(path)
	if errRead != nil {
		t.Fatalf("read converted agent identity auth file: %v", errRead)
	}
	var document map[string]any
	if errDecode := json.Unmarshal(data, &document); errDecode != nil {
		t.Fatalf("decode converted agent identity auth file: %v", errDecode)
	}
	...
	if _, exists := document["access_token"]; exists {
		t.Fatal("converted agent identity auth must not contain access_token")
	}
	...
	if runtime.GOOS != "windows" {
		...
		if errStat != nil {
			t.Fatalf("stat converted agent identity auth file: %v", errStat)
		}
		if got := info.Mode().Perm(); got != 0o600 {
			t.Fatalf("converted agent identity auth file mode = %o, want 600", got)
		}
	}

	auth, okAuth := manager.GetByID("codex-agent@example.com.json")
	if !okAuth {
		t.Fatal("converted agent identity auth was not registered")
	}
