...
func TestListAuthFilesCooldownsSnapshot(t *testing.T) {
	...
	for _, query := range []string{"", "?page=1&page_size=3"} {
		payload := requestAuthFilesCooldowns(t, h, query)
...
func TestListAuthFilesCooldownsUnknown(t *testing.T) {
	...
	for _, mode := range []string{"disk", "home"} {
		t.Run(mode, func(t *testing.T) {
			...
			if mode == "disk" {
				for _, query := range []string{"?auth_index=missing", "?auth_index=missing&page=1&page_size=3"} {
					if filtered := requestAuthFilesCooldowns(t, h, query); len(filtered.Files) != 0 {
