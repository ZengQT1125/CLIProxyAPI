...
func (s *GitTokenStore) pushRepositoryLocked(repo *git.Repository, repoDir string) error {
	...
	if errPush := pushWithRecover(repo, pushOpts); errPush != nil {
...
func pushWithRecover(repo *git.Repository, opts *git.PushOptions) (err error) {
	defer func() {
		if recovered := recover(); recovered != nil {
			err = fmt.Errorf("panic recovered in go-git push: %v", recovered)
		}
	}()
